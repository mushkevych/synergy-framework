...
SYNERGY_CLASSPATH="$SYNERGY_CLASSPATH:$HADOOP_LIB/commons-logging-1.1.1.jar"
...
SYNERGY_CLASSPATH="$SYNERGY_CLASSPATH:lib/gson-1.7.1.jar:synergy-framework-01.jar"
...
case "$1" in
  start)
    java $SYNERGY_JAVA_OPT -cp $SYNERGY_CLASSPATH com.reinvent.synergy.rest.RestServer 1>${JVM_OUT} 2>${JVM_ERR} &
    echo $! > $PIDFILE
    ;;
  stop)
